...
export default function ChatInterface({ isOnline, showToast }) {
  ...
  const handleSend = async (text = input) => {
    ...
    if (!isOnline) {
      showToast('Backend is offline. Please start the backend server.', 'error')
      return
    }
  ...
  const downloadChat = () => {
    if (messages.length === 0) {
      showToast('No chat to download yet', 'info')
      return
    }
    ...
    showToast('Chat session downloaded!', 'success')
  }
